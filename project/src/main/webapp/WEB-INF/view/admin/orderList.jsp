		...
		<c:forEach items="${saleList}" var="sale" varStatus="stat">
			...
			<tr style="background-color:pink;">
				...
				<td style="text-align: center;"><c:if
						test="${!((sale.s_step == 7)  || (sale.s_step == 9))}">
					...
					</c:if><br> <script type="text/javascript">
			...
			if (${sale.s_step == 9}) {disp${stat.index}.innerHTML = '반품/환불완료'}
				...
				<c:if test="${!((sale.s_step == 7) || (sale.s_step == 9))}">
						<button class="progress" value="${stat.index}">선택</button>
					</c:if></td>
			</tr>
			<tr>
				<td colspan="4">
						...
						<c:forEach items="${sale.saleItemList }" var="saleItem">
							<tr>
								<td rowspan="2"><img
									...
									height="100"></td>
								<td colspan="2"><h4>${saleItem.item.i_name }</h4></td>
							</tr>
							<tr>
							...
							<c:set var="tot"
								value="${tot+ saleItem.item.i_price* saleItem.quantity}" />
						</c:forEach>
						<c:set var="tot" value="${(tot - tot%100)*0.01 }" />
					</table> <input type="hidden" value="${tot }" id="tot${stat.index}">
				</td>
			</tr>
		</c:forEach>
	</table>
